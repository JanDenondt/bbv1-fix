...
namespace Big
{
	...
	void MainScript::Initialize()
	{
		...
		g_UiManager->AddSubmenu<RegularSubmenu>(BIG_TRANSLATE("Demo_sub"), SubmenuTest, [](RegularSubmenu* sub)
		{
			...
			sub->AddOption<BoolOption<bool>>("God Mode", nullptr, &Functions::godmode, BoolDisplay::OnOff);

			sub->AddOption<RegularOption>("Spawn T20", "Spawn T20 at your location", []
			{
					g_Logger->Info("T20 spawned.");

					Hash car = MISC::GET_HASH_KEY("t20");
					g_CallbackScript->AddCallback<ModelCallback>((car), [=] {
					

						auto pos = ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), true);
						*(unsigned short*)g_GameVariables->m_ModelSpawnBypass = 0x9090;
						Vehicle vehicle = VEHICLE::CREATE_VEHICLE(car, pos.x, pos.y, pos.z, 0.f, TRUE, FALSE, FALSE);
					...
					STREAMING::SET_MODEL_AS_NO_LONGER_NEEDED(car);
			});
 ...
 3);
	...
	{
		g_UiManager->OnTick();
		Functions::FunctionLoop(); // Load Features
	}
}
