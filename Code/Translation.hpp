...
namespace Big
{
	...
	public:
		...
		{
			//std::snprintf(&m_TranslationDir[0], sizeof(m_TranslationDir) - 1, "%s\\" VENOM_NAME "\\Translations\\", std::getenv("appdata"));
			try
			{
		...
		void LoadTranslations(const char* name)
		{
			...
			{
				char filename[100] = {};
				std::snprintf(&filename[0], sizeof(filename) - 1, "%s.txt", name);
				auto filePath = std::filesystem::path("C:\\BigBase\\Translations\\").append(filename);
